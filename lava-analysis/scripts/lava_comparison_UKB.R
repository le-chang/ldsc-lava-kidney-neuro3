# Description: run LAVA with UKB reference for PD_females vs uacr_female
# Load packages -----------------------------------------------------------
...
args <- list(
  ref_prefix = file.path(project_dir, "reference_data", "ukb_ref", "ukb_ref"),  # Update with your UKB reference path
  ...
  output_filename = "PD_females_vs_uacr_female_UKB"
)
...
# Print progress
cat(sprintf("\n=== Starting LAVA analysis with UKB reference for %d loci ===\n", n_loci))
...
out_dir <- file.path(project_dir, "sumstats", "kidney_neurodegen", "lava_results", "UKB_ref")
