...
#SBATCH --mem=8G
#SBATCH --array=0-7
#SBATCH --output=ldsc_h2-%A-%a.out
#SBATCH --error=ldsc_h2-%A-%a.err
...
MUNGED_DIR="/home/lchang24/scratch/munge_output"
OUTPUT_DIR="/home/lchang24/scratch/ldsc_h2_results"

# Create output directory if it doesn't exist
mkdir -p ${OUTPUT_DIR}

# Define munged sumstats files (8 files total)
MUNGED_FILES=(
    "${MUNGED_DIR}/PD_females.A1ref.munged.sumstats.gz"
    "${MUNGED_DIR}/PD_females.A1alt.munged.sumstats.gz"
    "${MUNGED_DIR}/PD_males.A1ref.munged.sumstats.gz"
    "${MUNGED_DIR}/PD_males.A1alt.munged.sumstats.gz"
    "${MUNGED_DIR}/PDnp_females.A1ref.munged.sumstats.gz"
    "${MUNGED_DIR}/PDnp_females.A1alt.munged.sumstats.gz"
    "${MUNGED_DIR}/PDnp_males.A1ref.munged.sumstats.gz"
    "${MUNGED_DIR}/PDnp_males.A1alt.munged.sumstats.gz"
)
